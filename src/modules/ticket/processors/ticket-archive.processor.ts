...
export class TicketArchiveProcessor {
  ...
  async processNewTicketNotification(
    job: Job<{ ticketId: string; userId: string; department: string }>,
  ) {
  ...
  async processTicketAssignmentNotification(
    job: Job<{
      ticketId: string;
      managerId: string;
      assignedBy: string;
  ...
  async processNewReplyMessageNotification(
    job: Job<{
      ticketId: string;
      messageId: string;
      senderId: string;
    ...
    const start = performance.now();
    try {
      const { ticketId, messageId, senderId, isFromClient, department } =
        job.data;
      ...
      this.logger.error(
        ...
        error.stack,
        'processNewReplyMessageNotification',
      );
      throw error;
    }
  }
